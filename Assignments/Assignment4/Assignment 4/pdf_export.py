...
def draw_instruction_grid(draw, chunk, box, cube_size=3):
...
def export_pdf(image, rows, cols, cube_size=3):
    """
    Generates PDF pages splitting the mosaic grid across printable pages.
    """
    if image is None:
        return []
    ...
    # Calculate grid patch dimensions based on cube_size
    patch_w = img_w // cols
    patch_h = img_h // rows

    # Create cover/overview page
    cover = image.resize((800, 800), Image.Resampling.NEAREST)
    draw = ImageDraw.Draw(cover)
    draw.text((20, 20), f"Cube Canvas Assembly Plan ({rows}x{cols} grid)", fill="white")
    ...
    for r in range(rows):
        for c in range(cols):
            ...
            crop_patch = image.crop((x1, y1, x2, y2))
            patch_large = crop_patch.resize((600, 600), Image.Resampling.NEAREST)

            page_canvas = Image.new("RGB", (800, 1000), "white")
            page_canvas.paste(patch_large, (100, 200))

            p_draw = ImageDraw.Draw(page_canvas)
            p_draw.text((100, 100), f"Section Row {r+1}, Column {c+1}", fill="black")
