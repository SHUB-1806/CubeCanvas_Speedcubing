...
print(image_processing.__file__)
import numpy as np
...
class App(ctk.CTk):
    def __init__(self):
        ...
        self.selected_algorithm = "nearest"
    ...
    def confirm_crop(self, event=None):
        ...
        self.generate_all_algorithms(
             self.cropped_image,
             rows,
             cols
        )

        self.processed_image = self.algorithm_results[self.algorithm]
    ...
    def apply_page3_effects(self):
        ...
        self.preview_image = apply_effects(
            ...
            negative=self.negative_switch.get(),
            white_balance_enabled=self.white_balance_switch.get(),
            gamma_enabled=self.gamma_switch.get(),
            grayscale_enabled=self.grayscale_switch.get()
        )
    ...
    def generate_all_algorithms(self, image, rows, cols):
        self.algorithm_results = {}
        ...
        for algorithm in self.algorithms:
            ...
            self.algorithm_results[algorithm] = result

    def change_algorithm(self, choice):
    ...
    def undo(self):
        if len(self.undo_stack) > 1:
            ...
            self.processed_image = self.undo_stack[-1].copy()
    ...
    def export_current_pdf(self):
        ...
        cube_size = int(self.cube_size_menu.get().split("×")[0])
    ...
    def display_dashboard_image(self, image, canvas):
    ...
    def select_dashboard_algorithm(self, algorithm):

        self.algorithm = algorithm
        self.selected_algorithm = algorithm
        self.processed_image = self.algorithm_results[algorithm]
    ...
    def open_dashboard(self):
        ...
        dashboard.title("Algorithm Comparison")
        ...
        for i, name in enumerate(names):
            ...
            algorithm_key = self.algorithms[i]

            frame = ctk.CTkFrame(grid)

            if algorithm_key == self.selected_algorithm:
                frame.configure(border_width=3, border_color="dodgerblue")
            ...
            canvas.bind(
                "<Button-1>",
                lambda event, alg=algorithm_key: self.select_dashboard_algorithm(alg)
            )
            ...
            image = self.algorithm_results[algorithm_key]
            ...
            dashboard.after(
                100,
                lambda img=image, c=canvas: self.display_dashboard_image(img, c)
            ) 
