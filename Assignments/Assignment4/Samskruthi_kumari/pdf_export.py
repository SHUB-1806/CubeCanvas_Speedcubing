...
def chunk_counts(image, chunk_size):
    # Ceiling division includes partial right/bottom edge chunks in the plan.
    return (
        math.ceil(image.width / chunk_size),
        math.ceil(image.height / chunk_size)
    )


def chunk_bounds(image, col, row, chunk_size):
...
def draw_chunk_grid(draw, image, placed_box, chunk_size, current=None, show_labels=True):
    ...
    cols, rows = chunk_counts(image, chunk_size)
    label_font = load_font(16, bold=True)

    for row in range(rows):
        for col in range(cols):
            x1, y1, x2, y2 = chunk_bounds(image, col, row, chunk_size)
            box = [
                x + x1 * scale_x,
                y + y1 * scale_y,
                x + x2 * scale_x,
...
def create_overview_page(image, chunk_size, cube_size):
    ...
    cols, rows = chunk_counts(image, chunk_size)
    cube_cols = math.ceil(image.width / cube_size)
    cube_rows = math.ceil(image.height / cube_size)
...
def draw_instruction_grid(draw, chunk, box, cube_size):
    ...
    for py in range(0, chunk.height + 1, cube_size):
        y = start_y + py * cell
        draw.line([(start_x, y), (start_x + grid_w, y)], fill=CUBE_LINE, width=5)

    for px in range(0, chunk.width + 1, cube_size):
        x = start_x + px * cell
...
def export_pdf(image, cube_size):
    # Normalize to RGB so every PDF page can be saved without mode-specific errors.
    image = image.convert("RGB")
    chunk_size = cube_size * STICKERS_PER_CUBE

    chunk_cols, chunk_rows = chunk_counts(image, chunk_size)
    pages = [create_overview_page(image, chunk_size, cube_size)]
