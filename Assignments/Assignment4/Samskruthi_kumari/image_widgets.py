...
from image_processing import show_histogram
...
def p3_topbar(app):
    ...
    app.cube_size_menu = ctk.CTkOptionMenu(
        app.p3_topbar,
        values=["3×3", "4×4", "5×5"]
    )

    app.cube_size_menu.set("3×3")

    app.cube_size_menu.grid(
        row=0,
        column=1,
        padx=(5, 10),
        pady=10
...
def p3_left_panel(app):
    ...
    app.negative_switch = ctk.CTkSwitch(
        switch_frame,
        text="Negative",
        command=app.apply_page3_effects
    )

    app.negative_switch.pack(
        anchor="w",
        padx=15,
        pady=(10, 15)
    )

    app.white_balance_switch = ctk.CTkSwitch(
        switch_frame,
        text="White Balance",
        command=app.apply_page3_effects
    )

    app.white_balance_switch.pack(
        anchor="w",
        padx=15,
        pady=(10, 15)
    )
    ...
    app.gamma_switch = ctk.CTkSwitch(
        switch_frame,
        text="Gamma Correction",
        command=app.apply_page3_effects
    )

    app.gamma_switch.pack(
        anchor="w",
        padx=15,
        pady=(10, 15)
    )
    ...
    app.grayscale_switch = ctk.CTkSwitch(
        switch_frame,
        text="Grayscale",
        command=app.apply_page3_effects
    )

    app.grayscale_switch.pack(
        anchor="w",
        padx=15,
        pady=(10, 15)
    )
    ...
    app.algorithm_menu = ctk.CTkOptionMenu(
        app.p3_left,
        values=["nearest", "floyd", "ordered", "atkinson"],
        command=app.change_algorithm
    )

    app.algorithm_menu.set("nearest")

    app.algorithm_menu.pack(
        fill="x",
        padx=10,
        pady=(0, 10)
    )

    app.histogram_button = ctk.CTkButton(
        app.p3_left,
        text="Show Histogram",
        command=lambda: show_histogram(app.preview_image)
    )

    app.histogram_button.pack(
        fill="x",
        padx=10,
        pady=(10, 10)
    )

    app.dashboard_button = ctk.CTkButton(
        app.p3_left,
        text="Algorithm Dashboard",
        command=app.open_dashboard
    )

    app.dashboard_button.pack(
        fill="x",
        padx=10,
        pady=(0, 10)
    )
