...
def p3_topbar(app):
    ...
    app.p3_topbar.grid_columnconfigure(1, weight=0)
    app.p3_topbar.grid_columnconfigure(2, weight=0)
    app.p3_topbar.grid_columnconfigure(3, weight=1)
    app.p3_topbar.grid_columnconfigure(4, weight=0)
    app.p3_topbar.grid_columnconfigure(5, weight=0)
    ...
    app.cube_menu = ctk.CTkOptionMenu(
        app.p3_topbar,
        values=["3x3", "4x4", "5x5"],
        command=lambda mode: app.apply_page3_effects()
    )
    ...
    app.hist_button = ctk.CTkButton(
        app.p3_topbar,
        text="Show Histogram",
        command=app.display_histogram
    )

    app.hist_button.grid(
        row=0,
        column=2,
        padx=(10, 5),
        pady=10,
    ...
    app.menu = ctk.CTkOptionMenu(
        app.p3_topbar,
        values=["Nearest", "Floyd-Steinberg", "Ordered Bayer", "Atkinson"],
        command=lambda mode: app.apply_page3_effects()
    )
...
def p3_left_panel(app):
    ...
    slider_configs = [
        ...
        ("Gamma Correction", "gamma_correction_slider", 0, 5, 1),
    ]
    ...
    for text, attr_name, min_val, max_val, default in slider_configs:
        ...
        # Map a responsive numeric slider onto the sub frame and hook runtime events directly to update handlers.
        ...
        slider.bind("<ButtonPress-1>", app.slider_pressed)
    ...
    app.flip_x_switch.bind("<Button-1>", app.switch_pressed)
    ...
    app.flip_y_switch.bind("<Button-1>", app.switch_pressed)

    app.invert_image_switch = ctk.CTkSwitch(
        switch_frame,
        text="Invert Image",
        command=app.apply_page3_effects
    )

    app.invert_image_switch.pack(
        anchor="w",
        padx=15,
        pady=10
    )

    app.invert_image_switch.bind("<Button-1>", app.switch_pressed)

    app.grayscale_image_switch = ctk.CTkSwitch(
        switch_frame,
        text="Grayscale Image",
        command=app.apply_page3_effects
    )

    app.grayscale_image_switch.pack(
        anchor="w",
        padx=15,
        pady=10
    )

    app.grayscale_image_switch.bind("<Button-1>", app.switch_pressed)

    app.white_balance_image_switch = ctk.CTkSwitch(
        switch_frame,
        text="White balance Image",
        command=app.apply_page3_effects
    )

    app.white_balance_image_switch.pack(
        anchor="w",
        padx=15,
        pady=10
    )

    app.white_balance_image_switch.bind("<Button-1>", app.switch_pressed)
    ...
    app.blur_mosaic_switch.bind("<Button-1>", app.switch_pressed)

    undo_frame = ctk.CTkFrame(app.p3_left)
    ...
    app.undo_button = ctk.CTkButton(
        undo_frame,
        text="Undo",
        command=app.undo
    )

    app.undo_button.pack(fill="x",expand= "True", padx=5, pady=5)

    app.redo_button = ctk.CTkButton(
        undo_frame,
        text="Redo",
        command=app.redo
    )

    app.redo_button.pack(fill="x",expand= "True", padx=5, pady=5)
...
def p2_image_display(app):
    ...
    app.image_canvas.bind("<B1-Motion>", app.move_crosshair,
                          add="+")  # without this + it will get confused between initial binding for move_CROSSHAIR
