...
from CubeCanvas.Rishikesh_J_250902.image_processing import *
import numpy as np
...
class App(ctk.CTk):
    ...
    def apply_page3_effects(self):
        ...
        self.preview_image = apply_effects(
            ...
            gamma=self.gamma_correction_slider.get(),
            flip_horizontal=self.flip_x_switch.get(),
            flip_vertical=self.flip_y_switch.get(),
            grayscale_img=self.grayscale_image_switch.get(),
            invert_img=self.invert_image_switch.get(),
            white_balance_img=self.white_balance_image_switch.get()
        )
    ...
    def revert_changes(self):
        ...
        self.gamma_correction_slider.set(1)
        ...
        self.grayscale_image_switch.deselect()
        self.invert_image_switch.deselect()
        self.white_balance_image_switch.deselect()
    ...
    def show_histogram(self):
        """
        Displays the RGB histogram of a PIL image.
        """
    ...
    def save_state(self):
        state = {
            ...
            "flip_x": self.flip_x_switch.get(),
            "flip_y": self.flip_y_switch.get(),
            "invert": self.invert_image_switch.get(),
            "grayscale": self.grayscale_image_switch.get(),
            "white": self.white_balance_image_switch.get(),
            "blur_mosaic": self.blur_mosaic_switch.get(),

            "algorithm": self.menu.get()
        }
    ...
    def restore_state(self, state):
        ...
        self.contrast_slider.set(state["contrast"])
        self.saturation_slider.set(state["saturation"])
        self.sharpness_slider.set(state["sharpness"])
        self.blur_slider.set(state["blur"])
        self.gamma_correction_slider.set(state["gamma"])

        self.flip_x_switch.select() if state["flip_x"] else self.flip_x_switch.deselect()
        self.flip_y_switch.select() if state["flip_y"] else self.flip_y_switch.deselect()
        self.invert_image_switch.select() if state["invert"] else self.invert_image_switch.deselect()
        self.grayscale_image_switch.select() if state["grayscale"] else self.grayscale_image_switch.deselect()
        self.white_balance_image_switch.select() if state["white"] else self.white_balance_image_switch.deselect()
        self.blur_mosaic_switch.select() if state["blur_mosaic"] else self.blur_mosaic_switch.deselect()

        self.menu.set(state["algorithm"])
    ...
    def run_all4(self):
        ...
        for name, algorithm in self.algorithms.items():
            ...
            self.output = algorithm(self.preview_image, self.palette)
    ...
    def dashboard(self):
        self.comparison = ctk.CTkToplevel(self)

        self.comparison.title("Algorithm Comparison")
        self.comparison.geometry("600x650")

        self.comparison.attributes("-topmost", True)

        self.comparison.after(100, lambda: self.comparison.attributes("-topmost", False))
        ...
        for i, (name, result) in enumerate(self.results.items()):
            ...
            button = ctk.CTkButton(
                ...
                image=photo,
                text="",
                command=lambda img=result["image"], alg=name:
                self.select_algorithm(alg, img, button)
            )

            button.pack(pady=5)

    def select_algorithm(self, name, image, button):
        rows = int(self.rows_entry.get())
        cols = int(self.cols_entry.get())
        ...
        if hasattr(app, "selected_button"):
